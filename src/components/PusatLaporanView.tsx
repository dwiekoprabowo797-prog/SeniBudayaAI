...
export const PusatLaporanView: React.FC<PusatLaporanViewProps> = ({
  ...
  const applyOfficialKop = (doc: jsPDF, title: string, isLandscape = false) => {
    ...
    const pemerintah = (config.Pemerintah || "PEMERINTAH KOTA TARAKAN").toUpperCase();
    const sekolah = (config.Nama_Sekolah || "SMP NEGERI 10 TARAKAN").toUpperCase();
    const alamat = config.Alamat_Sekolah || "Jl. Amal Lama RT. 4 Kel. Pantai Amal Kec. Tarakan Timur Kota Tarakan";
    const logoUrl = config.Logo_Kiri || config.Logo_Kanan || "https://lh3.googleusercontent.com/d/1i82llPU1_OoXCVIVQKOHVrRNkV1Cc5Ho";
  ...
  const applySignatures = (doc: jsPDF, lastY: number) => {
    ...
    const kepsek = config.Nama_Kepsek || "IBRAHIM, M.Pd.";
    const nipKepsek = config.NIP_Kepsek || "197802152008011021";
    const namaGuru = config.Nama_Guru || "Nama Guru Pengampu";
    const nipGuru = config.NIP_Guru || "198804242025211058";
    const tempat = config.Tempat_Tanda_Tangan || "Tarakan";
