...
export const CetakKartuQRView: React.FC<CetakKartuQRViewProps> = ({ siswaList, config }) => {
  ...
  const handleExportPDF = async () => {
    ...
    const namaSekolah = (config.Nama_Sekolah || "SMP NEGERI 10 TARAKAN").toUpperCase();
    const alamatSekolah = (config.Alamat_Sekolah || "Jl. Amal Lama RT. 4 Kel. Pantai Amal Kec. Tarakan Timur Kota Tarakan").toUpperCase();
    const kepsek = config.Nama_Kepsek || "IBRAHIM, M.Pd.";
    const nipKepsek = config.NIP_Kepsek || "197802152008011021";
    const tempatTTD = config.Tempat_Tanda_Tangan || "Tarakan";
